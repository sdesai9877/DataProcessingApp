...
public class UniqueCustomerIdPerGeoZone implements IReport<Integer> {
	...
	public Map<String, Integer> generateReport(List<ReportModel> reportModels) {
		...
		for (ReportModel reportModel : reportModels) {
			tempReport.computeIfAbsent(reportModel.getGeozone(), k -> new HashSet<>()).add(reportModel.getCustomerId());
		}
